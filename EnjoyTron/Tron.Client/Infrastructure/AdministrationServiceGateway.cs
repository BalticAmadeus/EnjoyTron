...
using Tron.AdminClient.AdminService;
    ...
    public class AdministrationServiceGateway : IAdministrationServiceGateway
    {
        ...
        public async Task LoginAsync()
        {
            ...
            using (var adminService = new AdminServiceClient(new BasicHttpBinding(), new EndpointAddress(ConnectionData.Url)))
            {
                ...
                var initLoginResp = await adminService.InitLoginAsync(initLoginReq);
                ...
                var completeReq = new CompleteLoginReq
                {
                    Auth = new ReqAuth
                    {
                        ...
                        AuthCode = GetAuthCode(
                            string.Format("{0}:{1}:{2}:{3}{4}", ConnectionData.TeamName, ConnectionData.Username, _sessionId,
                                sequenceNumber, ConnectionData.Password)),
                        SessionId = _sessionId,
                        SequenceNumber = sequenceNumber,
                    },
                    ChallengeResponse =
                ...
                var completeLoginResp = await adminService.CompleteLoginAsync(completeReq);
                ...
                var resp = await adminService.CreateObserverAsync(req);
        ...
        public async Task<IEnumerable<Game>> ListGamesAsync()
        {
            using (var adminService = new AdminServiceClient(new BasicHttpBinding(), new EndpointAddress(ConnectionData.Url)))
            {
                ...
                var games = listGamesResp.Games.Select(p => new Game
                {
                    GameId = p.GameId,
                    Label = p.Label,
                    State = p.State,
